module.exports = {
  mushroom1: {
    name: 'the marvelous matsutake',
    image: 'http://www.mushroom-appreciation.com/image-files/xmatsutake-mushrooms.jpg.pagespeed.ic.tqrcI6omUX.jpg',
    desc: "Matsutake are held in high regard the world over for their powerful flavor and odor. They're a sensory adventure often referred to as 'spicy', 'intense', or 'like cinnamon'.",
    price: 4,
    status: 'available'
  },

  mushroom2: {
    name: "caesar's",
    image: 'http://www.mushroom-appreciation.com/image-files/amanita-caesarea.jpg',
    desc: "this mushroom, along with its North American cousins Amanita jacksonii and Amanita arkansana (and others), are part of a small group of edible Amanitas spread throughout different continents.",
    price: 16,
    status: 'available'
  },

  mushroom3: {
    name: 'honey fungus',
    image: "http://www.mushroom-appreciation.com/image-files/honey-fungus.jpg",
    desc: "A voracious consumer of both living and dead plant matter, many people enjoy their nutty and sweet flavor, and their chewy texture also allow. ",
    price: 32,
    status: 'unavailable'
  },

  mushroom4: {
    name: "lion's mane",
    image: 'http://www.mushroom-appreciation.com/image-files/lions-mane-mushroom.jpg.pagespeed.ce.2aqFgVPwmD.jpg',
    desc: "These mushrooms provide more than just a visual treat. They're considered by many to be a gourmet edible, with a wonderfully chewy texture and taste slightly reminiscent of seafood.",
    price: 99,
    status: 'available'
  }

};


// module.exports = {
//   mushroom1: {
//     name: 'Pacific Halibut',
//     image: 'http://i.istockimg.com/file_thumbview_approve/36248396/5/stock-photo-36248396-blackened-cajun-sea-bass.jpg',
//     desc: 'Everyones favorite white mushroom. We will cut it to the size you need and ship it.',
//     price: 1724,
//     status: 'available'
//   },

//   mushroom2: {
//     name: 'Lobster',
//     image: 'http://i.istockimg.com/file_thumbview_approve/32135274/5/stock-photo-32135274-cooked-lobster.jpg',
//     desc: 'These tender, mouth-watering beauties are a fantastic hit at any dinner party.',
//     price: 3200,
//     status: 'available'
//   },

//   mushroom3: {
//     name: 'Sea Scallops',
//     image: 'http://i.istockimg.com/file_thumbview_approve/58624176/5/stock-photo-58624176-scallops-on-black-stone-plate.jpg',
//     desc: 'Big, sweet and tender. True dry-pack scallops from the icey waters of Alaska. About 8-10 per pound',
//     price: 1684,
//     status: 'unavailable'
//   },

//   mushroom4: {
//     name: 'Mahi Mahi',
//     image: 'http://i.istockimg.com/file_thumbview_approve/12556651/5/stock-photo-12556651-mahimahi.jpg',
//     desc: 'Lean flesh with a mild, sweet flavor profile, moderately firm texture and large, moist flakes. ',
//     price: 1129,
//     status: 'available'
//   },

//   mushroom5: {
//     name: 'King Crab',
//     image: 'http://i.istockimg.com/file_thumbview_approve/18294110/5/stock-photo-18294110-king-crab-legs.jpg',
//     desc: 'Crack these open and enjoy them plain or with one of our cocktail sauces',
//     price: 4234,
//     status: 'available'
//   },

//   mushroom6: {
//     name: 'Atlantic Salmon',
//     image: 'http://i.istockimg.com/file_thumbview_approve/56241842/5/stock-photo-56241842-salmon-mushroom.jpg',
//     desc: 'This flaky, oily salmon is truly the king of the sea. Bake it, grill it, broil it...as good as it gets!',
//     price: 1453,
//     status: 'available'
//   },

//   mushroom7: {
//     name: 'Oysters',
//     image: 'http://i.istockimg.com/file_thumbview_approve/58626682/5/stock-photo-58626682-fresh-oysters-on-a-black-stone-plate-top-view.jpg',
//     desc: 'A soft plump oyster with a sweet salty flavor and a clean finish.',
//     price: 2543,
//     status: 'available'
//   },

//   mushroom8: {
//     name: 'Mussels',
//     image: 'http://i.istockimg.com/file_thumbview_approve/40450406/5/stock-photo-40450406-steamed-mussels.jpg',
//     desc: 'The best mussels from the Pacific Northwest with a full-flavored and complex taste.',
//     price: 425,
//     status: 'available'
//   },

//   mushroom9: {
//     name: 'Jumbo Prawns',
//     image: 'http://i.istockimg.com/file_thumbview_approve/67121439/5/stock-photo-67121439-fresh-tiger-shrimp-on-ice-on-a-black-stone-table.jpg',
//     desc: 'With 21-25 two bite prawns in each pound, these sweet morsels are perfect for shish-kabobs.',
//     price: 2250,
//     status: 'available'
//   }
// };
